...
exports.getAll = async (req, res) => {
  try {
    const [rows] = await pool.query(
      'SELECT * FROM sr_expenses WHERE user_id=? ORDER BY expense_date DESC',
      [req.user.id]
    );
...
exports.create = async (req, res) => {
  ...
  try {
    const [result] = await pool.query(
      'INSERT INTO sr_expenses (user_id, amount, category, description, expense_date) VALUES (?,?,?,?,?)',
...
exports.remove = async (req, res) => {
  try {
    const [result] = await pool.query(
      'DELETE FROM sr_expenses WHERE id=? AND user_id=?',
